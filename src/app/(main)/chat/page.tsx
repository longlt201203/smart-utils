...
import { clearChat, getChat, getGeminiResponseText, pushChat } from "@/app/_actions/chat";
...
export default function ChatPage() {
    ...
    const fetchChat = async () => {
        ...
        setChat([firstMessage, ...data.map((item) => ({ name: item.name, content: item.content }))]);
    ...
    const handleSendMessage = async (message: string) => {
        ...
        await pushChat(data);
        await fetchChat();
        setChat((prev) => [...prev, { name: "Chat", content: (<Text>Getting response from Gemini...<Spinner/></Text>) }]);
        const result = await getGeminiResponseText({ message: message, history: chatHistory });
        ...
        <div className="w-full">
            ...
            <div className="flex flex-col gap-3">
                <Button
                    ...
                    type="secondary"
                    onClick={handleClearChat}>Clear Chat</Button>
